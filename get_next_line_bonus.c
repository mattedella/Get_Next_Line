...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/11/15 18:08:00 by mdella-r          #+#    #+#             */
/*   Updated: 2024/02/13 14:58:43 by mdella-r         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	char		*str;
	char		*buff;
	static char	*buff_stat[4096];

	if (fd < 0 || BUFFER_SIZE < 1 || read(fd, 0, 0) < 0)
	{
		if (buff_stat[fd] != NULL)
		{
			free(buff_stat[fd]);
			buff_stat[fd] = NULL;
		}
	...
	if (buff == NULL)
		return (NULL);
	if (ft_strchr(buff_stat[fd], '\n') == NULL)
		buff_stat[fd] = ft_fill(buff, buff_stat[fd], fd);
	free (buff);
	buff = NULL;
	str = ft_copy(buff_stat[fd]);
	buff_stat[fd] = ft_cut(buff_stat[fd]);
	return (str);
}
